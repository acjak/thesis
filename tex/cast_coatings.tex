...
\begin{figure}[htbp]
  ...
  \caption{\footnotesize Grh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhui}
...
\end{eqnarray}
The effective area and throughput for the optimised Pt/C coating can be seen in figures \ref{fig:pt-c_effara_throughput_lin} and \ref{fig:pt-c_effara_throughput_log}.

\begin{figure}[htbp]
\centering
\begin{minipage}{.47\textwidth}
  \centering  \includegraphics[width=\linewidth]{figures/cast/pt-c_effara_throughput_lin.pdf}
  \captionof{figure}{\footnotesize Grh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhui}
  \label{fig:pt-c_effara_throughput_lin}
\end{minipage}
\hspace{10pt}
\begin{minipage}{.47\textwidth}
  \centering  \includegraphics[width=\linewidth]{figures/cast/pt-c_effara_throughput_log.pdf}
  \captionof{figure}{\footnotesize Grh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhuiGrh gurehiuf wehui fwhguierhui}
...
\emph{The shipping of the substrates were done with DHL, but the package was lost between Denmark and US. A new batch of substrates were collected, cleaned, coated, packed and shipped during June/July of 2014. In this chapter is described the process for the second batch.}
...
!!!! Something about the 13 instead of 14 layers !!!!
...
A theodolite mounted 3 m from the front of the helioscope was aligned with the bore axis, so looking through the theodolite one could see the ball and hole of the optic. The optic rotation and translation could then be adjusted using the linear actuators to line up the ball and hole, and get a precise alignment. After the hole and ball was lined up, the optic was slid back into the vacuum vessel and the bolts were fastened.

!!!!!! Picture of hole and ball through theodolite !!!!!!!

The theodolite could be set to shine a laser beam with adjustable divergence through the bore and at the optic. The optic reflected the laser light nicely, making it possible to get an exact position for the detector as seen in figure \ref{fig:cast_align} (left). A fake detector with a carbonite back was used for the alignment. That allowed the laser light focused by the optic to shine at the carbonite back and be viewable from the outside.
...
In figure ???? can be seen the data output from the MicroMegas detectors after a 5 hour measurement with the COOL-X X-ray source. A flux of less than 1 Hz was seen. According to the effective area plot in figure \ref{fig:pt-c_effara_throughput_log}, the throughput at 8 keV is only $\sim$1\%.

Raytracing was done by Michael J. Pivovaroff from LLNL, and can be seen in figure ????. The figure assumes a 16 mm diameter source placed 14.2 m from the optic. They assume 100\% reflectivity from the mirror coatings and do not account for the spectrum of the COOL-X.
...
The measurement results obtained using the COOL-X X-ray source show that surplus slumped glass can be repurposed into well-behaving X-ray optics for specialised tasks.
